...
class SellerSecurityRevalidationService
{
    ...
    /**
     * @param  array<string, mixed>  $extraEligibility
     */
    public function handle(User $seller, string $reason, array $extraEligibility = []): int
    {
        if (! $seller->exists) {
            return 0;
        }

        $plans = DB::transaction(function () use ($seller, $reason, $extraEligibility): array {
     ...
     */
    private function eligibilitySnapshot(Store $store, User $seller, array $extra): array
    {
